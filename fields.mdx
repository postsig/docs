...
| PostSig Notes | `postsig_notes` | Internal notes field |
...
| Cancel By Date | `cancel_by_date` | The date range that determines the date by which a contract needs to be cancelled to avoid automatic renewal, expressed in days |
...
| Subscription Term | `subscription_term` | The length of the contract term |
...
| Product ID | `vendor_products.id` | Unique identifier for the product in the catalog |
...
| Percentage Increase Per Period | `annual_increase` | The amount a contract is scheduled to increase in the subsequent term expressed as a percentage |
